...
const PreferenceContextProvider: FC<Props> = ({ children }) => {
  ...
  const theme = themeFromSourceColor(
    argbFromHex(preferenceContextValue.preference.themeColor),
    [
      {
        name: 'budge-avatar',
  ...
  const styleElement: HTMLElement =
    document.querySelector('style#Material') || document.createElement('style')
  ...
  const scheme: (keyof typeof lightScheme)[] = [
    "primary",
    "onPrimary",
    "primaryContainer",
    "onPrimaryContainer",
  ...
  for (const k in scheme) {
    lightThemes.push(
      ...
      k +
      ':' +
      convert.hex.hsl(hexFromArgb(lightScheme[k])).join(' ') +
      ';',
    )
  }
  for (const k in darkScheme) {
    darkThemes.push(
      '--' +
      k +
      ':' +
      convert.hex.hsl(hexFromArgb(darkScheme[k])).join(' ') +
      ';',
    )
  }
  for (const k in theme.palettes) {
    for (let i = 0; i <= 100; i++) {
      ...
      lightThemes.push(
        ...
        i +
        ':' +
        convert.hex.hsl(hexFromArgb(color)).join(' ') +
        ';',
      )
    }
  }
  theme.customColors.forEach((e) => {
    for (const k in e.light) {
      lightThemes.push(
        ...
        k +
        ':' +
        convert.hex.hsl(hexFromArgb(e.light[k])).join(' ') +
        ';',
      )
    }
    for (const k in e.dark) {
      darkThemes.push(
        ...
        k +
        ':' +
        convert.hex.hsl(hexFromArgb(e.dark[k])).join(' ') +
        ';',
      )
    }
  })
  ...
  styleElement.innerText =
    ':root{' + lightThemes.join('') + '}:root.dark{' + darkThemes.join('') + '}'
